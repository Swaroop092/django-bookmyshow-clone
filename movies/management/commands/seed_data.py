...
class Command(BaseCommand):
    help = 'Idempotently seed application data from data_clean.json'

    def handle(self, *args, **options):
        fixture_path = os.path.join(settings.BASE_DIR, 'data_clean.json')
        if not os.path.exists(fixture_path):
            self.stdout.write(self.style.ERROR(f"Fixture file not found at {fixture_path}"))
            return

        self.stdout.write("Loading clean fixture data...")
        ...
        # 4. Seed Movie (and Many-to-Many fields)
        ...
        for item in movies:
            ...
            if 'genres' in fields:
                movie.genres.set(fields['genres'])
            if 'languages' in fields:
                movie.languages.set(fields['languages'])
